...
namespace gui {
	...
	inline void Empty::draw(sf::RenderTarget& target) {}
	void Empty::update() {}
	...
	void Widget::update() {
		...
		frontOut.setColor(th.boxOutline);
		frontOut.update();
		// scroll panel
		if (scroll)
			scrollPanel->update();
	...
	void Widget::handlesSetup() {
		...
		sprDragL.setColor(th.boxOutline);
		sprDragT.setColor(th.boxOutline);
	}
	...
	void Channels::recalculate() {
		...
		// visual fix (add at the end if outside bounds)
		...
		for (unsigned int y = 0U; y < height; y++)
			for (unsigned int x = 0U; x < width; x++)
				rectOut[width * y + x].setColor(th.boxOutline);
		for (unsigned int y = 0U; y < height; y++)
			for (unsigned int x = 0U; x < width; x++) {
	...
	void Channels::refreshText() {
		...
		for (unsigned int i = 0U; i < width * height; i++) {
			unsigned int px = std::ceil(rect[i].pos.x / cs.patFull - 0.5f) + offx;
	...
	void Channels::update() {
		...
		sx = sx - int(sx);
		sy = sy - int(sy);
		app::cslog("scroll: " + std::to_string(sx) + " " + std::to_string(sy), "");
		sx = std::floor(sx * cs.patFull);
		sy = std::floor(sy * cs.patFull);
		for (unsigned int y = 0U; y < height; y++)
			for (unsigned int x = 0U; x < width; x++) {
				rect[width * y + x].setPos(x * cs.patFull - sx + cs.patSpace, (y - 1) * cs.patFull - sy + cs.patSpace);
				rect[width * y + x].update();
			}
		for (unsigned int y = 0U; y < height; y++)
			for (unsigned int x = 0U; x < width; x++) {
				rectOut[width * y + x].setPos(x * cs.patFull - sx + cs.patSpace, (y - 1) * cs.patFull - sy + cs.patSpace);
				rectOut[width * y + x].update();
			}
		for (unsigned int y = 0U; y < height; y++)
			for (unsigned int x = 0U; x < width; x++)
				text[width * y + x].setPosition(x * cs.patFull - sx + cs.patSpace, (y - 1) * cs.patFull - sy + cs.patSpace);
	}

	void Channels::draw(sf::RenderTarget& target) {
		for (unsigned int y = 0U; y < height; y++)
			for (unsigned int x = 0U; x < width; x++) {
				rect[width * y + x].draw(target);
			}
		for (unsigned int y = 0U; y < height; y++)
			for (unsigned int x = 0U; x < width; x++) {
				rectOut[width * y + x].draw(target);
			}
		for (unsigned int y = 0U; y < height; y++)
			for (unsigned int x = 0U; x < width; x++) {
				target.draw(text[width * y + x]); // TODO: (OPTIMIZATION, 46%) cache to a texture on refreshText() and draw that instead
			}
	}
